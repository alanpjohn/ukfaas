...
func NewInternalNetworkService(opts ...any) (networkapi.NetworkService, error) {

	networkManager := InternalNetworkService{}
	for _, opt := range opts {
		ncOpt, ok := opt.(InternalNetworkServiceOption)
		if !ok {
			return nil, fmt.Errorf("invalid InternalNetworkService option : %v", opt)
		}
...
func (n *InternalNetworkService) AddServiceIP(_ context.Context, service string, ip networkapi.IP) error {
	var (
		lb  LoadBalancer
		ok  bool
		err error
	...
	n.instanceCount += 1
	log.Printf("[InternalNetworkService.AddServiceIP] - Added IP %s to Service %s", ip, service)
	return nil
}

func (n *InternalNetworkService) DeleteServiceIP(_ context.Context, service string, ip networkapi.IP) error {
	var (
		lb LoadBalancer
		ok bool
	)
	...
	n.instanceCount -= 1
	log.Printf("[InternalNetworkService.DeleteServiceIP] - Deleted IP %s from Service %s", ip, service)
	return nil
}

func (n *InternalNetworkService) DeleteService(_ context.Context, service string) error {
	var (
		lb LoadBalancer
		ok bool
	)
	if existingLb, exists := n.instancesMap.LoadAndDelete(service); exists {
		...
		log.Printf("[InternalNetworkService.DeleteService] - Deleted Service %s", service)
	}

	return fmt.Errorf("service %s not found", service)
}

func (n *InternalNetworkService) ResolveServiceEndpoint(service string) (*url.URL, error) {
...
func (n *InternalNetworkService) AvailableIPs(_ context.Context, service string) (uint64, error) {
	var (
		lb    LoadBalancer
		ok    bool
		count uint64
...
func (n *InternalNetworkService) RunHealthChecks(ctx context.Context) {
	for {
